...
import { getSubscription, cancelSubscription, getLastFourDigitsOnCard, customerInvoices, getMinutesUsedInPeriod, type IGetSubscription } from '../api/stripe';
...
type SubscriptionWithProduct = IGetSubscription & {
    product?: IProduct & {
        name?: string;
        active?: boolean;
    };
...
const Billing: React.FC = () => {
    ...
    const [invoiceDate, setInvoiceDate] = useState<number | null>(null);
    ...
    useEffect(() => {
        const fetchInvoices = async () => {
            try {
                const _invoices = await customerInvoices() as InvoicesResponse | Invoice[];
                
                let invoicesData: Invoice[] = [];
                
                // Handle different response structures
                if (Array.isArray(_invoices)) {
                    invoicesData = _invoices;
                } else if (_invoices && typeof _invoices === 'object') {
                    // Check if it has invoices.data structure
                    if ('invoices' in _invoices && _invoices.invoices?.data) {
                        invoicesData = _invoices.invoices.data;
                    }
                    // Check if it has direct data array
                    else if ('data' in _invoices && Array.isArray(_invoices.data)) {
                        invoicesData = _invoices.data;
                    }
                }
    ...
    useEffect(() => {
        const fetchSubscription = async () => {
            ...
            try {
                const sub = await getSubscription() as SubscriptionWithProduct | null;
                setSubscription(sub);
                
                // Safely access product name with proper type checking
                const productName = sub?.product?.name;
                const currentPlan = plans.find(p => p.name === productName);
    ...
    const updateBillingDataFromSubscription = (sub: SubscriptionWithProduct | null) => {
        if (sub && sub.product) {
            ...
            setBillingData(prev => ({
                ...
                planPrice: formatPlanPrice(product) || "$0/month",
    ...
    const formatPlanPrice = (product: IProduct | undefined) => {
        if (!product?.name) return "$0/month";
        
        if (product.name === "Queen Plan") {
            return "$300/month";
        } else if (product.name === "King Plan") {
            return "$600/month";
        } else if (product.name === "Emperor Plan") {
            return "$2000/month";
        } else if (product.name === "Knight Plan") {
            return "$50/month";
        }
        return "$0/month";
    };

    const calculateNextBillingDate = (invoiceTimestamp: number | null) => {
    ...
    const statusConfig: {
        ...
        inactive: { color: string; text: string; };
        canceled: { color: string; text: string; };
        past_due: { color: string; text: string; };
    } = {
    ...
    const getStatusBadge = (status: keyof typeof statusConfig): JSX.Element  => {
