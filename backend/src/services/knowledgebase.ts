import fs from "fs";
import { clientRetell } from "./retell.js";
  ...
  try {
    ...
    try {
      ...
      if (!orgError && orgData && orgData.length > 0) {
        ...
        orgKBs = orgKnowledgebaseNames.map((kbName: string) => {
        ...
        }).filter((kb: { knowledge_base_id: any; }) => kb.knowledge_base_id); // Filter out any that weren't found in Retell
      }
    } catch (orgErr) {
...
export const listOrgKnowledgebases = async (orgId: string): Promise<KnowledgeBase[]> => {
  try {
    ...
    return orgKnowledgebaseNames.map((kbName: string) => {
      const kbDetails = allRetellKBs.find(kb => kb.knowledge_base_name === kbName);
      return {
        knowledge_base_id: kbDetails?.knowledge_base_id || '',
        knowledge_base_name: kbName,
    ...
    }).filter((kb: { knowledge_base_id: any; }) => kb.knowledge_base_id);
  } catch (error) {
